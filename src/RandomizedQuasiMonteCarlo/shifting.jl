...
"""
function digital_shift(rng::AbstractRNG, points::AbstractArray, b::Integer; M = 32)
    ...
    for i in CartesianIndices(random_points)
        random_points[i] = bits2unif(@view(bits[:, i]), b)
    end

    return random_points
end

digital_shift(points::AbstractArray, b::Integer; M = 32) = digital_shift(Random.default_rng(), points, b; M = M)
...
digital_shift_bits!(random_bits::AbstractMatrix{<:Integer}, b::Integer) = digital_shift_bits!(Random.default_rng(), random_bits, b)
